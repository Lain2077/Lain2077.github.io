...
$(document).ready(function() {
    ...
    addIconare('areyou/areu.html', 'layer', `C:/LastLine/questions.bat`, `images/oc-img/comm.png`, 'Are you human?', 4);
...
function addIcon(page, name, address, icon, text, row) {
    let $icon = $(`
    <div class="dLink"><button class="button" onclick="deployWindow('${page}', '${name}', '${address}')">
...
function addIconare(page, name, address, icon, text, row) {
    let $icon = $(`
    <div class="dLink"><button class="button" onclick="deployWindow('${page}', '${name}', '${address}');audio.play()">
        <div class="dIcon" style="background-image: url('${icon}' );"></div>
        ${text}
    </button></div>
    `);
