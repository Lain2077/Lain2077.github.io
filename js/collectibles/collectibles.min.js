...
function CollectiblesJS(e) {
    ...
    function u(e) {
        ...
        (i.style.visibility = "hidden"),
            ...
            b.animate &&
                b.animate(
                    { transform: ["translate(" + m + "px, " + u + "px)", "none"] },
    ...
    for (let e of Array.from(document.getElementsByClassName("collectible-item"))) {
        const t = document.createElement("div");
        (t.className = "collectible-item-inner"),
            e.appendChild(t),
            (e.title = "Click me!"),
            (e.alt = "Collectible item (Click me!)"),
            m[e.dataset.keyid] || (e.style.visibility = "visible"),
            e.addEventListener("click", u);
    }
    document.body.insertAdjacentHTML(
        "beforeend",
        '\n    <div id="collectible-item-drawer">\n        <div id="collectible-item-chest-holder">\n            <a id="collectible-secret-link">\n                <div id="collectible-item-chest"></div><br><br>\n                Click me </a>\n        </div>\n        <div id="collectible-item-holder">\n        </div>\n    </div>'
    );
    ...
    b.insertAdjacentHTML(
        "beforeend",
        '<div class="collectible-help" aria-label="Find all ' +
            i +
            's !" data-microtip-position="' +
            { left: "right", right: "left" }[s] +
            '" role="tooltip"></div>'
    ),
        d(m);
}
    ...
    function u(t) {
        function n(e, t, r, n) {
            var i,
                o = 0,
                s = "";
            for (t && (i = t.length - 1); o < i || s.length < e; )
                (s += c(Math.abs(parseInt(t[o], r)).toString(2), n)), o++;
        ...
        function a(e) {
            var t,
                r = null;
            for (10, 32, t = Math.ceil(e / 32); null === r; )
                r = n(e, window.crypto.getRandomValues(new Uint32Array(t)), 10, 32);
            return r;
        }
        function l(e) {
            var t;
            if ((10, 32, (t = Math.ceil(e / 32)), !sjcl.random.isReady(o)))
                  ...
                  : "object" == typeof e && "function" == typeof e.randomBytes
                    ...
                    : !window ||
                        ...
                        ("function" != typeof window.crypto.getRandomValues &&
                            "object" != typeof window.crypto.getRandomValues) ||
                        ("function" != typeof Uint32Array && "object" != typeof Uint32Array)
    ...
    var g = {
        init: function (e, n) {
            var i,
                ...
                h = [],
                u = 1;
            if ((a(), e && ("number" != typeof e || e % 1 != 0 || e < t.minBits || e > t.maxBits)))
                throw new Error(
                    "Number of bits must be an integer between " + t.minBits + " and " + t.maxBits + ", inclusive."
                );
            if (n && -1 === s.indexOf(n)) throw new Error("Invalid RNG type argument : '" + n + "'");
            for (
                r.radix = t.radix,
                    r.bits = e || t.bits,
                    r.size = Math.pow(2, r.bits),
                    r.maxShares = r.size - 1,
                    i = t.primitivePolynomials[r.bits],
                    c = 0;
                c < r.size;
                c++
            )
            ...
            if (
                ...
                n && this.setRNG(n),
                l() || this.setRNG(),
                "browserSJCLRandom" === r.typeCSPRNG &&
                    ((sjcl.random = new sjcl.prng(o)), sjcl.random.startCollectors()),
                !(
                    l() &&
                    r.bits &&
                    r.size &&
                    ...
                    r.logs &&
                    r.exps &&
                    r.logs.length === r.size &&
                    r.exps.length === r.size
                ))
            )
        ...
        combine: function (e, t) {
            ...
            for (t = t || 0, n = 0, s = e.length; n < s; n++) {
                ...
                if ((r.bits !== l && this.init(l), -1 === w.indexOf(u.id)))
                    for (i = w.push(u.id) - 1, o = 0, a = (f = m(d(u.data))).length; o < a; o++)
                        (g[o] = g[o] || []), (g[o][i] = f[o]);
            }
        ...
        getConfig: function () {
            ...
            return (
                (e.radix = r.radix),
                (e.bits = r.bits),
                (e.maxShares = r.maxShares),
                (e.hasCSPRNG = l()),
                (e.typeCSPRNG = r.typeCSPRNG),
                e
            );
        },
        extractShareComponents: function (e) {
            var n,
                ...
                l = {};
            if (
                (n = parseInt(e.substr(0, 1), 36)) &&
                ("number" != typeof n || n % 1 != 0 || n < t.minBits || n > t.maxBits)
            )
                throw new Error(
            ...
            if (
                ...
                (a = new RegExp("^([a-kA-K3-9]{1})([a-fA-F0-9]{" + o + "})([a-fA-F0-9]+)$").exec(e)) &&
                    (i = parseInt(a[2], r.radix)),
        ...
        setRNG: function (e) {
            ...
            if (e && "string" == typeof e && -1 === s.indexOf(e))
                throw new Error("Invalid RNG type argument : '" + e + "'");
            if ((e || (e = u()), e && "string" == typeof e && (e = u(e)), i)) {
                ...
                if (e && !parseInt(e(r.bits), 2))
                    throw new Error(t + "(Binary string output not parseable to an Integer)." + n);
                if (e && e(r.bits).length > r.bits)
                    throw new Error(t + "(Output length is greater than config.bits)." + n);
                if (e && e(r.bits).length < r.bits)
                    throw new Error(t + "(Output length is less than config.bits)." + n);
            }
            return (r.rng = e), !0;
        },
        str2hex: function (e, r) {
            ...
            if ((r || (r = t.bytesPerChar), "number" != typeof r || r < 1 || r > t.maxBytesPerChar || r % 1 != 0))
                throw new Error(
                    "Bytes per character must be an integer between 1 and " + t.maxBytesPerChar + ", inclusive."
                );
            for (n = 2 * r, i = Math.pow(16, n) - 1, a = 0, l = e.length; a < l; a++) {
                if (((s = e[a].charCodeAt()), isNaN(s))) throw new Error("Invalid character: " + e[a]);
                if (s > i)
                    throw (
                        ((o = Math.ceil(Math.log(s + 1) / Math.log(256))),
                        new Error(
        ...
        hex2str: function (e, r) {
            ...
            if ("number" != typeof (r = r || t.bytesPerChar) || r % 1 != 0 || r < 1 || r > t.maxBytesPerChar)
                throw new Error(
                    "Bytes per character must be an integer between 1 and " + t.maxBytesPerChar + ", inclusive."
                );
            for (i = 0, o = (e = c(e, (n = 2 * r))).length; i < o; i += n)
                s = String.fromCharCode(parseInt(e.slice(i, i + n), 16)) + s;
            return s;
        },
        random: function (e) {
            if ("number" != typeof e || e % 1 != 0 || e < 2 || e > 65536)
                throw new Error("Number of bits must be an Integer between 1 and 65536.");
            if ("browserSJCLRandom" === r.typeCSPRNG && sjcl.random.isReady(o) < 1)
                throw new Error(
        ...
        share: function (e, t, n, i) {
            ...
            if ("number" != typeof i || i % 1 != 0 || i < 0 || i > 1024)
                throw new Error("Zero-pad length must be an integer between 0 and 1024 inclusive.");
            for (a = 0, u = (e = m((e = "1" + d(e)), i)).length; a < u; a++)
                for (s = p(e[a], t, n), l = 0; l < t; l++)
                    (f[l] = f[l] || s[l].x.toString(r.radix)), (b[l] = c(s[l].y.toString(2)) + (b[l] || ""));
            for (a = 0; a < t; a++) f[a] = w(r.bits, f[a], h(b[a]));
            return f;
        },
        newShare: function (e, t) {
            if ((e && "string" == typeof e && (e = parseInt(e, r.radix)), e && t && t[0]))
                return w(this.extractShareComponents(t[0]).bits, e, this.combine(t, e));
